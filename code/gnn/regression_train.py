...
def run_an_eval_epoch(args, model, data_loader):
    model.eval()
    eval_meter = Meter()
    test_smiles, test_labels, test_prediction = [], [], []
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            smiles, bg, labels, masks = batch_data
    ...
    return [np.mean(eval_meter.compute_metric(args['metric'])), test_smiles, test_labels, test_prediction]
...
def main(args, exp_config, train_set, val_set, test_set):
    ...
    for epoch in range(args['num_epochs']):
        ...
        val_score = run_an_eval_epoch(args, model, val_loader)[0]
    ...
    # this part is modified to output y_prediction
    test_run = run_an_eval_epoch(args, model, test_loader)
    test_score = test_run[0]
    with open(args['trial_path'] + '/test_output.csv' ,'w') as f:
        f.write('SMILES,LABELS,PREDICTIONS\n')
        for i in range(len(test_run[1])):
            for j in range(len(test_run[1][i])):
                f.write('{},{},{}\n'.format(test_run[1][i][j],float(test_run[2][i][j]),float(test_run[3][i][j])))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--split',
                        choices=['scaffold_decompose', 'scaffold_smiles', 'random', 'consecutive_smiles'],
    ...
    if args['split'] != 'consecutive_smiles':
        train_set, val_set, test_set = split_dataset(args, dataset)
    else:
        cv_dataset = split_dataset(args, dataset)[0]
    ...
    # use split by "consecutive_smiles" will automatically induce the 10-fold CV which data slicing is equivalent to what used to feed the sklearn models
    elif args['split'] == 'consecutive_smiles':
        exp_config = get_configure(args['model'])
        for fold in cv_dataset:
            train_set, test_set = fold[0], fold[1]
            main(args, exp_config, train_set, test_set, test_set)
